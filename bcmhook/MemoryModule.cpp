    ...
    for (i=0; i<module->headers->FileHeader.NumberOfSections; i++, section++) {
        ...
        if (size > 0) {
            ...
            if (VirtualProtect((LPVOID)((POINTER_TYPE)section->Misc.PhysicalAddress | imageOffset), size, protect, &oldProtect) == 0)
                //printf("Error protecting memory page\r\n");
#ifdef DEBUG_OUTPUT
        ...
        for (; relocation->VirtualAddress > 0; ) {
            ...
            for (i=0; i<((relocation->SizeOfBlock-IMAGE_SIZEOF_BASE_RELOCATION) / 2); i++, relInfo++) {
                ...
                case IMAGE_REL_BASED_ABSOLUTE:
                    //printf("IMAGE_REL_BASED_ABSOLUTE: %d\n", type);
                    // skip relocation
                    break;
...
HMEMORYMODULE MemoryLoadLibrary(const void *data,DWORD * dwBase)
{
    return MemoryLoadLibraryEx(data, _LoadLibrary, _GetProcAddress, _FreeLibrary, NULL,dwBase);
}
    ...
    void *userdata,DWORD * dBase)
{
    ...
    dwBase = result->headers->OptionalHeader.ImageBase;
